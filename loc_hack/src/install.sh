...
/usr/java/bin/cvm -Xms16m -classpath bcel-5.2.jar:K3Translator.jar Translator tj4 ${ORIGFOLDER_JAR}/PictureViewer.jar translation.jar ${ORIGFOLDER_LIB} en_GB bg >> ${INST_LOG} 2>&1
mv ${ORIGFOLDER_LIB}/PictureViewer.jar ${ORIGFOLDER_LIB}/PictureViewer-bg_BG.jar >> ${INST_LOG} 2>&1

logmsg "I" "update" "Translate JARs"
update_progressbar 20
/usr/java/bin/cvm -Xms16m -classpath bcel-5.2.jar:K3Translator.jar Translator td4 ${ORIGFOLDER_LIB} translation.jar en_GB bg >> ${INST_LOG} 2>&1
...
sed -i 's/^locale=.*$/locale=bg-BG/' /var/local/java/prefs/com.amazon.ebook.framework/prefs
sed -i 's/^low_level_screens.dir=.*$/low_level_screens.dir=\/opt\/amazon\/low_level_screens\/bg\/600x800/' /var/local/java/prefs/com.amazon.ebook.framework/prefs
sed -i 's/^clippings.file.name=.*$/clippings.file.name=\u041c\u043e\u0457 \u0432\u0438\u0440\u0456\u0437\u043a\u0438.txt/' /var/local/java/prefs/com.amazon.ebook.framework/prefs
/usr/bin/install_boot_images /opt/amazon/low_level_screens/bg/600x800

update_progressbar 80
logmsg "I" "update" "Setup dictionaries"
echo "dictionary.list = bg,bg-BG" > /opt/amazon/dict/conf/bg.conf
